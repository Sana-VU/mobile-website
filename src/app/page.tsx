        ...
        <section className="w-full max-w-4xl text-center mb-12 relative">
          ...
          <div className="relative px-8 py-12 md:py-16">
            ...
            <p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto leading-relaxed">
              Your ultimate destination for mobile phone reviews, comparisons, and buying guides. 
...
function Card({
  ...
  const backgroundGradients = {
    ...
    compare: "from-blue-50 to-blue-100/50 dark:from-blue-950/30 dark:to-blue-900/20",
    article: "from-violet-50 to-violet-100/50 dark:from-violet-950/30 dark:to-violet-900/20"
  };

  const icons: Record<string, React.ReactNode> = {
    grid: <LucideGrid className={`w-8 h-8 ${iconColors.grid} transition-all duration-300 group-hover:scale-110`} />,
    compare: <GitCompare className={`w-8 h-8 ${iconColors.compare} transition-all duration-300 group-hover:scale-110`} />,
    article: <LucideFileText className={`w-8 h-8 ${iconColors.article} transition-all duration-300 group-hover:scale-110`} />,
  };
  ...
  return (
    ...
    >
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${
        icon === 'grid' ? 'from-emerald-400 to-emerald-600' :
        icon === 'compare' ? 'from-primary to-blue-600' :
